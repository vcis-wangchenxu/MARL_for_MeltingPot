...
from models import AgentDRQN, VDNMixer
from memories import OffPolicyReplayBuffer
from MeltingPotWrapper import build_meltingpot_env as GeneralMeltingPotWrapper

class VDNAgent:
    """ Value Decomposition Networks (VDN) Agent for Multi-Agent Reinforcement Learning in MeltingPot Environments. """
    def __init__(self, env_info: dict, args: Dict):
        """ Initialize the VDN agent with environment information and hyperparameters. """
        self.env_info = env_info                          # Environment information dictionary
        self.n_agents = env_info["n_agents"]              # Number of agents in the environment
        self.n_actions = env_info["n_actions"]            # Number of possible actions per agent
        self.obs_shape = env_info["obs_shape"]            # Observation space shape per agent
        self.agent_ids = env_info["agent_ids"]            # List of agent IDs

        self.args = args                                  # Hyperparameters and configurations     
        self.lr = args["lr"]                              # Learning rate
        self.gamma = args["gamma"]                        # Discount factor
        self.rnn_hidden_dim = args["rnn_hidden_dim"]      # RNN hidden dimension
        self.grad_norm_clip = args["grad_norm_clip"]      # Gradient norm clipping
        ...
        self.mixer_network = VDNMixer().to(self.device)
    ...
    def take_action(self,
        ...
        for i, agent_id in enumerate(self.agent_ids):
            agent_q_values = q_values[0, i, :]          # (n_actions,)
    ...
    def _get_agent_q_values(self, 
        ...
        """ Get Q-values for the taken actions of all agents. """
    ...
    def update(self, batch: Dict[str, np.ndarray]) -> float:
        """ Update the VDN agent/learner networks. """
        ...
        with torch.no_grad():    # Target Q calculation
            ...
            target_q_total = self.target_mixer_network(target_max_q_values)    # (B, L)
        ...
        current_q_total = self.mixer_network(chosen_action_q_values)        # (B, L)
...
def evaluate(vdn_agent: VDNAgent, 
                env: GeneralMeltingPotWrapper, 
                n_episodes: int) -> float:
    """ Evaluate the VDN agent over n_episodes and return average reward. """
    total_reward = 0.0
    vdn_agent.agent_network.eval() 

    for _ in range(n_episodes):
        obs_dict, state = env.reset()
        hidden_states_dict = vdn_agent.init_hidden()
        done = False
        
        while not done:
            actions_dict, next_hidden_states_dict = vdn_agent.take_action(
...
def train_VDN(vdn_agent: VDNAgent,
               env: GeneralMeltingPotWrapper, 
               config: dict,
               replay_buffer: OffPolicyReplayBuffer) -> Tuple[List[Dict], List[Dict]]:
    """ Train the VDN agent in the given environment using the specified configuration and replay buffer. """
    print("--- Start VDN Training ---")
    ...
    run_name = config.get('run_name', f"VDN_{config['env_name']}_{config['seed']}")
    ...
    while total_steps < config['total_timesteps']:
        ...
        vdn_agent.agent_network.train()
        vdn_agent.mixer_network.train()
        ...
        if i_episode % config['eval_freq'] == 0:
            eval_reward = evaluate(vdn_agent, env, n_episodes=3) 
            ...
            if eval_reward > best_eval_reward:
                ...
                torch.save(vdn_agent.get_networks_state_dict(), save_path)

    env.close()
    print("--- VDN Training Finished ---")
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        print(f"\n===== Starting VDN execution with Seed: {seed} =====\n")
        run_name = f"VDN-MeltingPot-{seed}-{time.strftime('%Y%m%d-%H%M%S')}"
        
        run = swanlab.init(
            project="VDN-MeltingPot-MultiSeed-v1", 
        ...
        multi_agent = VDNAgent(env_info, config)
        
        print(f"Using device: {config['device']}")

        single_run_data, single_run_data_eval = train_VDN(multi_agent, env, config, replay_buffer)
    ...
    multi_algo_data['VDN'] = all_seeds_data_list
    multi_algo_data_eval['VDN'] = all_seeds_data_eval_list

    plot_learning_curve(experiments_data=multi_algo_data, 
                        title="VDN Training Performance on MeltingPot", 
                        output_filename="vdn_meltingpot_training_curve.png")
    plot_learning_curve(experiments_data=multi_algo_data_eval, 
                        title="VDN Evaluation Performance on MeltingPot", 
                        output_filename="vdn_meltingpot_evaluation_curve.png")